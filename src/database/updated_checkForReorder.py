...
def predictDemand(SKU):
    ...
    prediction_data['Day'] = pd.to_datetime(data['Date']).dt.day
    prediction_data['Month'] = pd.to_datetime(data['Date']).dt.month
    prediction_data['Year'] = pd.to_datetime(data['Date']).dt.year
